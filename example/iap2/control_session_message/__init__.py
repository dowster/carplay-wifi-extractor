...
async def read_csm(reader):
    ...
    start, length, msg_id = CSM_STRUCT.unpack(header_bytes)
    print(f"CSM header: {header_bytes.hex()}")
    if start != CSM_START:
        return
    payload = await reader.readexactly(length - 6)
    print(f"CSM payload: {payload.hex()}")
    ...
    def decorator(clazz):
        def build_deserialize_params(handlers):
            def deserialize_params(self, payload):
                ...
                # print(f"CSM handlers initial: {handlers_clone}")
                while len(payload) > 0:
                    length, param_id = CSM_PARAM_STRUCT.unpack(
                        payload[:4])
                    param_payload = payload[4:length]
                    payload = payload[length:]
                    print(f"CSM param: id={param_id:#x} len={length} payload={param_payload.hex()}")
                    if param_id in handlers_clone:
                        ...
                        print(f"CSM param deserialize: name={name} list={is_list} value={value}")
                        if is_list:
                            value_list = []
                ...
                for _serializer, _deserializer, name, is_list, is_optional in handlers_clone.values():
                    if not is_optional and not is_list:
                        ...
                        print(f"[debug] {name}, {is_optional}, {is_list}")
                    setattr(self, name, [] if is_list else None)
                # print(f"CSM handlers remaining: {handlers_clone}")
