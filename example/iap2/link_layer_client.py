__all__ = ["ClientIAP2Connection", "IAP2Stream"]
...
from typing import ClassVar, List, Callable, Any, Optional

MARKER_DEBUG_FILE = "iap2_marker_bytes_client.bin"


def _reset_marker_log():
    try:
        with open(MARKER_DEBUG_FILE, "wb"):
            pass
    except OSError:
        pass


def _append_marker_bytes(data: bytes):
    try:
        with open(MARKER_DEBUG_FILE, "ab") as marker_log:
            marker_log.write(data)
    except OSError:
        pass

def _format_hex_ascii(data: bytes):
    hex_str = data.hex()
...
class IAP2Stream:
    def __init__(self, conn: "ClientIAP2Connection", session_id: int, stream_id: int = None):
        self.conn = conn
        self.session_id = session_id
...
class ClientIAP2Connection:
    CONTROL_SESSION_ID = 0x01
    EA_SESSION_ID = 0x02
    ...
    def __init__(self,
        ...
        self.state = None
        client_lsp = LinkSynchronizationPayload(
            ...
            max_ack=1,
            sessions=[
                LSPSession(id=ClientIAP2Connection.CONTROL_SESSION_ID,
                           type=0,
                           version=2),
                LSPSession(id=ClientIAP2Connection.EA_SESSION_ID, type=2, version=2)
            ])
        self._client_lsp = client_lsp
        self.lsp = client_lsp
        ...
        self.remote_lsp: Optional[LinkSynchronizationPayload] = None
        ...
        self.control_session = IAP2Stream(self,
                                          ClientIAP2Connection.CONTROL_SESSION_ID)
        self.ea_streams = dict()
        self._receive_loop_task = None

    def create_ea_stream(self, stream_id):
        stream = IAP2Stream(self, ClientIAP2Connection.EA_SESSION_ID, stream_id)
    ...
    async def close(self):
        """Send a link-layer RESET and give the IVI time to react before closing."""
        try:
            # if self.state not in (None, STATE_DEAD):
            #     self._write_packet(seq=self._sent_psn, control=CONTROL_RST)
           
            # Proactively advertise iAP2 support so the IVI doesn't wait for
            # us while we wait for it.
    ...
    def _send_reset(self, reason: str = ""):
        try:
            self._write_packet(seq=self._sent_psn, control=CONTROL_RST)
            if reason:
                print(f"Client sent RESET due to: {reason}")
            else:
                print("Client sent RESET")
        except Exception as exc:
            print(f"Failed to send RESET: {exc}")

    async def bruteforce_rst(self, delay: float = 0.0):
        """Send RST packets sweeping all seq/ack combinations (0x00-0xFF)."""
        print("[rst-brute] Starting brute-force of seq/ack values", flush=True)
        for seq in range(0x100):
            for ack in range(0x100):
                header = LinkPacketHeader(control=CONTROL_RST,
                                          length=9,
                                          seq=seq,
                                          ack=ack,
                                          session_id=0)
                try:
                    self._output.write(header.pack())
                    if delay:
                        await asyncio.sleep(delay)
                except Exception as exc:
                    print(f"[rst-brute] Failed at seq={seq:#04x} ack={ack:#04x}: {exc}")
                    return
        try:
            await self._output.drain()
        except AttributeError:
            pass
        print("[rst-brute] Completed full seq/ack scan", flush=True)
    ...
    def _send_negotiate(self, include_ack=True):
        if self.state != STATE_NEGOTIATE:
            return
        lsp_bytes = self.lsp.pack()
        control = CONTROL_SYN | (CONTROL_ACK if include_ack else 0)
        self._write_packet(lsp_bytes, self._sent_psn, control, session_id=0)
        self._syn_sent = True

    async def _receive_loop(self):
        try:
            print(f"Waiting for IAP2 marker")
            _reset_marker_log()
            recv_marker = await self._input.readexactly(len(IAP2_MARKER))
            print(f"MKR: {recv_marker.hex()}")
            _append_marker_bytes(recv_marker)
            iter = 6
            while recv_marker != IAP2_MARKER:
                next_byte = await self._input.readexactly(1)
                _append_marker_bytes(next_byte)
                recv_marker = recv_marker[1:] + next_byte
                # if iter == 20:
                #     await self.bruteforce_rst(delay=0.2)
                # iter += 1
        ...
        #     self._send_reset("KeyboardInterrupt")
        #     self._bailout(None)
        #     raise
    ...
    def _handle_syn(self, lsp: LinkSynchronizationPayload, psn: int):
        print("Accessory:", lsp)
        print("Device:", self.lsp.sessions)
        self.lsp = lsp
        self._last_received_in_sequence_psn = psn
        self._last_acked_psn = psn
            
        if self.state == STATE_NEGOTIATE:
            if not self._syn_sent:
                self._send_negotiate(include_ack=True)
                # self._sent_psn = signed_add(self._sent_psn, 1)
        #         self.state = STATE_STATE_IDE
        # elif self.state == STATE_STATE_IDE:
        #     self._send_ack_start_ide()
    ...
    def _received_data(self, p: IAP2Packet):
        if p.session_id == ClientIAP2Connection.CONTROL_SESSION_ID:
            self.control_session.received_data(p.data)
        elif p.session_id == ClientIAP2Connection.EA_SESSION_ID and len(p.data) >= 2:
