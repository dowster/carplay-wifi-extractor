...
class TestLinkSynchronizationPayload(unittest.TestCase):
    ...
    def test_parse_ivi_syn_payload(self):
        header_bytes = bytes.fromhex("ff5a001a80e8000025")
        payload_bytes = bytes.fromhex("017fffff07d003e81e01010002020102")
        checksum = 0x99

        header = LinkPacketHeader.from_bytes(header_bytes)
        self.assertIsNotNone(header)
        self.assertEqual(header.length, 0x001a)
        self.assertEqual(header.control, CONTROL_SYN)
        self.assertEqual(header.seq, 0xE8)
        self.assertEqual(header.ack, 0)
        self.assertEqual(header.session_id, 0x25)
        print("IVI SYN header:", header)

        self.assertEqual(gen_checksum(payload_bytes), checksum)
        lsp = LinkSynchronizationPayload.from_bytes(payload_bytes)
        self.assertIsNotNone(lsp)
        self.assertEqual(lsp.max_outgoing, 0x7F)
        self.assertEqual(lsp.max_len, 0xFFFF)
        self.assertEqual(lsp.retransmission_timeout, 0x07D0)
        self.assertEqual(lsp.ack_timeout, 0x03E8)
        self.assertEqual(lsp.max_retransmissions, 0x1E)
        self.assertEqual(lsp.max_ack, 0x01)
        self.assertEqual(
            lsp.sessions,
            [
                LSPSession(id=0x01, type=0x00, version=0x02),
                LSPSession(id=0x02, type=0x01, version=0x02),
            ]
        )
        print("IVI SYN payload:", lsp)
