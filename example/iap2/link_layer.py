...
MARKER_DEBUG_FILE = "iap2_marker_debug.log"


def _log_marker_bytes(prefix: str, data: bytes):
    try:
        with open(MARKER_DEBUG_FILE, "a") as marker_log:
            marker_log.write(f"[SERVER] {prefix}: {data.hex()}\n")
    except OSError:
        pass
...
class IAP2Connection:
    ...
    async def _receive_loop(self):
        try:
            ...
            print(f"Waiting for IAP2 marker, initial bytes={recv_marker.hex()}")
            _log_marker_bytes("initial", recv_marker)
            if recv_marker != IAP2_MARKER:
                _log_marker_bytes("mismatch", recv_marker)
                self._bailout("IAP2 not supported")
                return
            print("IAP2 marker received")
